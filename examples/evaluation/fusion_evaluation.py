...
def main():
    ...
    parser.add_argument("--results_filepath", type=str, nargs="+", help="The file path to the results to evaluate")
    parser.add_argument(
    ...
    for result_filepath in args.results_filepath:
        logging.info(f"Loading results from {result_filepath}...")
        retrieval_results_list.append(util.load_runfile(result_filepath))

    fusion_results = util.fusion(
        retrieval_results_list,
        method="normalized_sum",
        depth=max(args.k_values)
    )
    ...
    util.save_runfile(os.path.join(results_dir, f"{args.topic}.run.trec"), fusion_results)
